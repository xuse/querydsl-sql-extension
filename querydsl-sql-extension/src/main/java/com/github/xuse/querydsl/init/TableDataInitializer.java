...
public class TableDataInitializer {
	...
	private int insertBatch(SQLQueryFactory session, List<Object> batch) {
		...
		try {
			...
			if(isSpringDuplicateKeyException(e)) {
				count = insertOneByOne(session, batch, e);	
			}else {
				throw e;
			}
	...
	private boolean isSpringDuplicateKeyException(RuntimeException e) {
		String name=e.getClass().getName();
		return "org.springframework.dao.DataIntegrityViolationException".equals(name) ||"org.springframework.dao.DuplicateKeyException".equals(name); 
	}
