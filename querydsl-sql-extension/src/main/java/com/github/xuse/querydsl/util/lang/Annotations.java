...
public class Annotations {
	...
	@FunctionalInterface
	public interface Attrib<T> extends Function<T,Object>, Serializable{
	}
	
	/**
	 * A Builder to create annotation instances.
	 * @param <T> the type of annotation.
	 */
	public static class Builder<T extends Annotation>{
		private final Class<T> annotationClz;
		private final Map<String,Object> map=new HashMap<>();
		private final long millis=System.currentTimeMillis();
		Builder(Class<T> clz){
			this.annotationClz=clz;
		}
		public Builder<T> setDateOffsetMillis(Attrib<T> attrib,TLDateFormat df,long offset){
			Date date=new Date(millis + offset);
			return set(attrib, df != null ? df.format(date) : date);
		}
		public Builder<T> set(Attrib<T> attrib,Object value){
			Pair<Class<?>, String> pair = Lambdas.analysis(attrib);
			map.put(pair.getSecond(), value);
			return this;
		}
		public Builder<T> set(String attrib,Object value){
			map.put(attrib, value);
			return this;
		}
		public T build() {
			return asAnnotation(annotationClz, map);
		}
	}
...
//            throw ((ExceptionProxy) result).generateException();
		...
		/**
		 * Returns an object's invocation handler if that object is a dynamic proxy with
		 * a handler of type AnnotationInvocationHandler. Returns null otherwise.
		 */
		 ...
		 * equal using the appropriate equality function depending on the member type.
		 * The two values will be of the same type unless one of the containing
		 * annotations is ill-formed. If one of the containing annotations is
		 * ill-formed, this method will return false unless the two members are
		...
		/**
		 * Returns the member methods for our annotation type. These are obtained lazily
		 * and cached, as they're expensive to obtain and we only need them if our
		 * equals method is invoked (which should be rare).
		 */
