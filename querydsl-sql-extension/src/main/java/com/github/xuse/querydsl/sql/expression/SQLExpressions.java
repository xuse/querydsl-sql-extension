...
public final class SQLExpressions {
    ...
    public static <T> Expression<T> setValues(Path<T> target) {
    	return Expressions.operation(target.getType(), SQLOps.SET_PATH, target, target);
    }
    ...
    public static <T> Expression<T> set(Path<T> target, Expression<? extends T> value) {
        if (value != null) {
            return Expressions.operation(target.getType(), SQLOps.SET_LITERAL, target, value);
        } else {
