...
public class PathMapping extends AbstractColumnMetadataEx implements ColumnMapping {
	...
	private static final Map<Class<?>,AbstractType<?>> CUSTOM_VARCHAR_TO_ARRAY_TYPES = new HashMap<>();
	static {
		CUSTOM_VARCHAR_TO_ARRAY_TYPES.put(String.class, new StringArrayAsVarcharType());
		CUSTOM_VARCHAR_TO_ARRAY_TYPES.put(int.class, new IntArrayAsVarcharType());
		CUSTOM_VARCHAR_TO_ARRAY_TYPES.put(long.class, new LongArrayAsVarcharType());
		CUSTOM_VARCHAR_TO_ARRAY_TYPES.put(float.class, new FloatArrayAsVarcharType());
		CUSTOM_VARCHAR_TO_ARRAY_TYPES.put(double.class, new DoubleArrayAsVarcharType());
	}
	...
	public PathMapping(Path<?> path, AccessibleElement field, ColumnMetadata column) {
		...
		CustomType customTypeAnno = field.getAnnotation(CustomType.class);
		...
		if (fieldType.isPrimitive()) {
			...
			if (column.isNullable() && customTypeAnno == null) {
		...
		if (spec != null) {
			...
			for(SpecialSpec spDef:spec.special()) {
				map.put(spDef.dbType(), spDef.value());
			}
			this.specialSpec = map.isEmpty()? Collections.emptyMap(): map;
		}
		Comment comment = field.getAnnotation(Comment.class);
		if (comment != null) {
			this.comment = comment.value();
		}
		if (customTypeAnno != null) {
			@SuppressWarnings("rawtypes")
			Class<? extends Type> clz = toTypeClass(customTypeAnno, field);
			if(clz!=null) {
				try {
					customType = SQLTypeUtils.createInstance(clz, customTypeAnno.parameters(), path.getType(),field.getGenericType());
			...
			customType =  tryAddAutoMapping(column.getJdbcType(),fieldType,path);
		}
	}
	...
	/*
	 * Auto generate for enum types and etc...
	 */
	...
	public PathMapping copyForPath(Path<?> newPath) {
		...
		p.specialSpec = this.specialSpec;
