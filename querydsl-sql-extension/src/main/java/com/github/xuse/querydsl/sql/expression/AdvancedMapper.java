...
public class AdvancedMapper extends AbstractMapperSupport implements Mapper<Object> {
	...
	private Map<Path<?>, Object> createMapOptimized(RelationalPathEx entity, Object bean, Object[] values) {
		...
		for (int i = 0; i < len; i++) {
			...
			// 如果具有自动生成类型，解析并（按需）回写
			Object autoValue = resolveAutoGenerated(metadata, nullValue, scenario, bean);
			if (autoValue != NO_AUTO_VALUE) {
				value = autoValue;
				nullValue = false;
			}
			if (nullValue) {
	...
	public static final AdvancedMapper ofNullsBinding(int scenario) {
		return ofNullsBinding(scenario, false);
	}
	
	public static final AdvancedMapper ofNullsBinding(int scenario, boolean ignoreKeys) {
		return new AdvancedMapper(scenario, ignoreKeys) {
	 ...
	 * @return AdvancedMapper
	 */
	public static final AdvancedMapper ofNullsAsDefaultBinding(int scenario, boolean ignoreKeys) {
		return new AdvancedMapper(scenario, ignoreKeys) {
			protected void processNullBindings(Path<?> path, List<Entry<Path<?>, Object>> data,
					ColumnMapping metadata) {
				if (isKeyColumn(path)) {
