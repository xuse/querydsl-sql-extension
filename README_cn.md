...
**querydsl-sql 扩展，在原版基础上提供了更高的性能和更丰富的功能。**
...
| Case                                                     | Mybatis 3.5.9（单位ms）                | querydsl-sql-extension<br /> 5.0.0-r172（单位ms） | A/B     |
| -------------------------------------------------------- | -------------------------------------- | ------------------------------------------------- | ------- |
...
动态类生成方案中，每一个SQL查询的SELECT字段组合对应一个访问器，在第一次查询时生成并加载，第二次执行相同SQL时性能就相当于硬编码。过程中去除了反射和IF分支，对ResultSet的全部访问均为按index的顺序访问（有利于CPU分支预测），大幅减少内存操作次数。

* **表模型与BeanCodec缓存**：对表和字段模型，以及每个对象的编解码器进行了缓存。

* **面向字节码 / JIT友好**： 采用栈上操作、尽可能final化、手工内联、对象复用、内存一次分配、用tableswitch代替复杂分支等编程技巧。总体原则是减少内存拷贝、精简字节码操作、对JIT友好、面向分支预测优化。

* **向开发者提供Tuning API**： 提供fetchSize, maxRows，queryTimeout等方法，操作大量数据时可根据业务需要做性能和安全的Tuning。
...
* 使用ASM9的动态类生成，支持JDK 8~ 22，GraalVM（22）。
...
* 包扫描：前文中已经介绍了几种基于Annotation的功能增强。包扫描可以在应用启动时分析所有数据库元模型定义，提前校验这些Annotation的正确性，并将其中的配置注册到全局上下文。注意：@CustomType需要通过包扫描才能生效。
...
完整的数据库结构建模：扩展QueryDSL原生的ColumnMetadata，支持在Java(JDBC)模型中描述全部数据库表的特征——包括default value、unsigned、自增、索引、约束（不含外键）。相关模型可以通过元模型API创建，也可以通过注解来定义。
  ...
  A: 数据库应用开发者一般有两种做法：一种是先在数据库中设计数据结构，然后通过代码生成工具生成Java类（称为Schema优先）；
  另一种是先建立Java Entity模型，然后通过程序自动创建数据库结构（称为Metadata优先）。典型地，Hibernate就支持启动时自动创建数据库结构；本框架也支持这一用法。

QueryDSL的设计思路是Schema优先，本框架扩展支持了Metadata优先的场景，可以通过Java元模型反向更新数据库结构。同时，这种更新支持基于数据库结构对比的增量更新，包括更新索引、约束等表结构。
...
参见  [querydsl-sql-r2dbc](querydsl-sql-r2dbc/) 和 [querydsl-sql-r2dbc-spring](querydsl-sql-r2dbc-spring/) 用法。
...
Online DDL可以避免DDL执行期间的锁阻塞对用户DML操作的影响。Online DDL指的是在DDL执行期间，允许用户继续进行DML操作。
...
* 此功能并不意味着DDL执行对数据表完全无影响，24小时运行的高可用系统仍应在业务低谷期执行DDL。
...
2. 如果我的项目已经用了querydsl-sql，现在集成这个框架，需要将原有的query class全部修改为继承`com.github.xuse.querydsl.sql.RelationalPathBaseEx`类吗？
    ...
    A: 可以不用。本框架是在querydsl上的一些轻微调整和改进，querydsl的原生用法都不受影响。

## 其他

### 什么是元模型/Query Class

* 元模型(meta model)：在QueryDSL中，对每个实体会有一个"Q"开头的class，在querydsl文档中，称为 `Query Class`. 其实这个类和JPA中的元模型用途差不多，都是用于描述数据结构，并且提供查询API引用的类。（在OpenJPA中，会生成下划线结尾的类，用途是差不多的）。所以本文某些场合也会使用元模型一词，和QueryDSL文档中的 query class是一个东西。
* 本框架自r104起，支持通过Lambda表达式代替元模型，从而省去`Query Class`。本质上是一种语法糖，但对初学者更加友好。
* Lambda表达式在表示数据表时是单例的，无法直接构造自表关联查询。
  （类似这个SQL语句，就是一个自表关联查询 `SELECT t.* FROM tree_node t LEFT JOIN tree_node parent ON t.pid = parent.id`）
  r110版本进一步扩展了API，支持使用Lambda表达式构建自表关联查询。
