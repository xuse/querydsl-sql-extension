...
动态类生成方案，每一个SQL查询的SELECT字段组合对应一个访问器，在第一次查询时生成并加载，第二次执行该SQL时性能就相当于硬编码。过程中去除了反射和IF分支，对ResultSet的全部访问均为按index的顺序访问（考虑CPU分支预测），大幅减少内存操作次数。
...
* 提供更多Batch Insert、Batch Update、Batch Delete操作。通过一个SQL Statement支持多组操作值，提升操作效率。
...
* 此功能的应用并不意味着DDL执行对数据表无影响，24小时运行的高可用系统还是应当在业务低谷期间执行DDL
