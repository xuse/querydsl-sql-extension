...
- 某一天，我这样想到：querydsl-jpa作为查询构建器可以复杂数据库访问，而JPA能提供的无非就是单表增删改查、级联和延迟加载功能。
  用querydsl把单表增删改封装成以JPA风格几乎没有难度。对于级联和延迟加载功能似乎也不是非要不可，那我还留着JPA干啥？
...
2018：性能演进
既然打算今后长期使用了，对性能吹毛求疵的我自然不能接受原生的反射操作，于是进行了优化。除了代码细节修改外，还增加了基于ASM自动生成动态类来完成字段拼装对象，无反射调用。
