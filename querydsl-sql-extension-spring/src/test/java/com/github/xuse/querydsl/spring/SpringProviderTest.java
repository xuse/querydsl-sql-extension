...
import com.github.xuse.querydsl.sql.spring.util.DbInvoke;
...
public class SpringProviderTest {
    ...
    @Test
    public void testDbInvoker() {
        DbInvoke db=DbInvoke.create(dataSource);
        db.fetch("select * from test_entity").iterator((r)->{
       ...
       long count=db.fetch("select * from test_entity").iterator((r)->{
            String s=r.getString("name");
            return s;
        })
        .toStream().count();
    ...
    public void testRepository() {
        for(int i=0;i<100;i++){
            ...
            a.setName("刘备"+r.nextInt(1_000_000));
    ...
    public void test1() {
        ...
        b.setName("王五");
        b.setVersion(1);
        factory.insert(QTestEntity.testEntity).populate(b).addBatch().populate(c).addBatch().batchToBulk(true).execute();
...
//		}
        ...
        metadata.refreshTable(QTestEntity.testEntity).dropColumns(true).dropConstraint(true).dropIndexes(true).simulate(false)
                .execute();
