...
- [快速入门](#快速入门)
	...
	- [5 扩展用法注解](#5-扩展用法注解)
		...
		- [@ConditionBean详解](#conditionbean详解)
	- [6. 包扫描功能](#6-包扫描功能)
		- [自动扫描并初始化数据库](#自动扫描并初始化数据库)
		- [确保仅初始化一次](#确保仅初始化一次)
		- [分布式锁](#分布式锁)
	- [7. 执行DDL语句](#7-执行ddl语句)
		- [表修改示例](#表修改示例)
		- [分区操作示例](#分区操作示例)
		- [权限问题](#权限问题)
		- [MySQL Online DDL（部分支持）](#mysql-online-ddl部分支持)
...
GenericRepository中提供了对单表常用的增删改查功能见下表，其他个性化业务逻辑可以在Repository类中自行编码实现。
...
上述代码可以快速获得数据库访问对象，但会失去一些高级特性。完整的初始化方式参见[第3章 使用手册](#3-使用手册)。
### 查询API介绍
...
参见此文档 [Without Springframework](without_springframework.md)
...
* 由于Record对象是不可变对象，所有字段都无法set，所以@AutoGenerated的回写功能会自动失效。（日志中会提示回写的数值被丢弃，但不会抛出异常）
...
configuration.getScanOptions()
	...
	.canCreateMissingTable(true) //允许创建缺失的表
    .withDataInitBehavior(DataInitBehavior.FOR_ALL_TABLE) //为所有扫描到的实体做数据初始化
...
> 虽然框架会使用Online方式来修改已有的数据库表，但仍有可能DDL执行会锁表。对可用性要求特别高的高负载生产环境，应当关闭启动时执行DDL的特性。
...
该表以业务表的名称作为主键。如果表已经初始化过了，is_disabled列值就被更新为1，下次启动后就不会反复初始化同一张表了。
...
> MySQL Online DDL从5.x开始支持，到8.x完善。需要根据具体环境判断控制Online DDL的范围。笔者手头的MySQL环境是5.6的，所以按5.6进行了适配。
>
> 8.x的优化还未进行。
