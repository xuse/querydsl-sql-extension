...
public class ObjectNameBuilder {
    ...
    public ObjectNameBuilder(Object mbean) throws MalformedObjectNameException {
        Class<? extends Object> mBeanClass = mbean.getClass();
        MBean annotation = mBeanClass.getAnnotation(MBean.class);
        if (annotation == null) {
            throw new IllegalArgumentException(mBeanClass + " is not annotated with " + MBean.class);
        }
        ...
        if (name == null || name.equals("")) {
            this.objectName = new ObjectName(mBeanClass.getPackage().getName() + ":name=" + mBeanClass.getSimpleName()
                    + "-" + mbean.hashCode());
        } else {
            this.objectName = ObjectName.getInstance(mBeanClass.getPackage().getName() + ":name=" + name + "-"
                    + mbean.hashCode());
        }
    }
    ...
    public ObjectName build() throws MalformedObjectNameException {
        return (objectName != null) ? objectName : ObjectName.getInstance(domain, new Hashtable<String, String>(
                properties));
    }
}
