...
public abstract class AbstractCrudRepository<T, ID> implements CRUDRepository<T, ID> {
	...
	public int countByExample(T example) {
		...
		for (Map.Entry<Path<?>, Object> entry : AdvancedMapper.DEFAULT.createMap(getPath(), example)
				.entrySet()) {
	...
	List<T> find(Predicate... where) {
		SQLQueryFactory factory = getFactory();
		return factory.selectFrom(getPath()).where(where).fetch();
	}
	...
	T load(Predicate... where) {
		SQLQueryFactory factory = getFactory();
		return factory.selectFrom(getPath()).where(where).fetchOne();
	}
	...
	public int deleteByExample(T t) {
		...
		for (Map.Entry<Path<?>, Object> entry : AdvancedMapper.DEFAULT.createMap(getPath(), t)
				.entrySet()) {
			SimpleExpression<?> p = (SimpleExpression<?>) entry.getKey();
			query.where(p.eq(ConstantImpl.create(entry.getValue())));
		}
	...
	public int updateByKeys(T obj, Path<?>... bizKeys) {
		...
		SQLQueryFactory factory = getFactory();
		Map<Path<?>, Object> map = AdvancedMapper.DEFAULT.createMap(getPath(), obj);
	...
	int count(Predicate... where) {
		return (int) getFactory().selectFrom(getPath()).where(where).fetchCount();
	}
	...
	/**
	 * Needs the SQLQueryFactory
	 */
	protected abstract SQLQueryFactory getFactory();

	/**
	 * The RelationalPath(path of the table).
	 */
