...
public class PathMapping extends AbstractColumnMetadataEx implements ColumnMapping{
	...
	public PathMapping(Path<?> path, Field field, ColumnMetadata column) {
		this(path, new AccessorImpl(field), column);
	}

	public PathMapping(Path<?> path,Accessor field, ColumnMetadata column) {
		super(column);
		this.field = field;
		this.path = path;
		...
		if(field.getType().isPrimitive()) {
			...
			if(column.isNullable() && anno==null && !ConfigurationEx.FREE_PRIMITIVE) {
				...
				log.info("Field {} using primitive type to mapping a column in database. Please using @UnsavedValue or withUnsavePredicate() method to describe it. otherwise the value like '0' or 'false' will be regarded as a null value.",field);
			}
		}
