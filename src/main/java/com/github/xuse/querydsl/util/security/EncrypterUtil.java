...
public class EncrypterUtil {
	...
	public enum AlgorithmType {
		...
		Signature, // 数字签名算法
		AlgorithmParameterGenerator, AlgorithmParameters, CertificateFactory, CertPathBuilder, CertPathValidator, Mac,
		Policy, SaslClientFactory, SaslServerFactory, SecretKeyFactory, SecureRandom, SSLContext, TerminalFactory,
		TransformService, TrustManagerFactory, XMLSignatureFactory
	}
	...
	public static byte[] decrypt(InputStream in, Key key, AlgorithmParameterSpec spec) {
		...
		try {
			...
			Cipher c1 = Cipher.getInstance(
					(key instanceof RawSecretKeySpec) ? ((RawSecretKeySpec) key).chiperAlgomrithm : key.getAlgorithm());
	...
	public static PasswordEncryptor getDefaultPBE() {
		return new PasswordEncryptor(new byte[] { (byte) 0xA9, (byte) 0x9B, (byte) 0xC8, (byte) 0x32, (byte) 0x56,
				(byte) 0x35, (byte) 0xE3, (byte) 0x03 }, 19);
	}
